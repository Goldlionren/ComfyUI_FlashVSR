...
# FlashVSR 模型根目录
weights_FlashVSR_current_path = os.path.join(folder_paths.models_dir, "FlashVSR")
if not os.path.exists(weights_FlashVSR_current_path):
    os.makedirs(weights_FlashVSR_current_path)

# 注册 FlashVSR 模型路径
folder_paths.add_model_folder_path("FlashVSR", weights_FlashVSR_current_path)
...
class FlashVSR_SM_Model(io.ComfyNode):
        ...
        return io.Schema(
            ...
            inputs=[
                io.Combo.Input(
                    ...
                    + [
                        i
                        for i in folder_paths.get_filename_list("FlashVSR")
                        if "dmd" in i.lower()
                    ],
                ),
                io.Combo.Input(
    ...
    @classmethod
    def execute(
        cls, dit, proj_pt, emb_pt, vae, tcd_encoder, tiny_long, decode_vae
    ) -> io.NodeOutput:
        dit_path = (
            folder_paths.get_full_path("FlashVSR", dit) if dit != "none" else None
        )
        proj_pt_path = (
            folder_paths.get_full_path("FlashVSR", proj_pt)
            if proj_pt != "none"
            else None
        )
        vae_path = (
            folder_paths.get_full_path("vae", vae) if vae != "none" else None
        )
        tcd_encoder_path = (
            folder_paths.get_full_path("FlashVSR", tcd_encoder)
            if tcd_encoder != "none"
            else None
        )
        prompt_path = (
            folder_paths.get_full_path("FlashVSR", emb_pt)
            if emb_pt != "none"
            else None
        )
        ...
        ), "Please select the emb (prompt tensor .pth)"
        assert (
            dit_path is not None and proj_pt_path is not None
        ), "Please select both DIT checkpoint (dmd) and proj_pt"
        assert (
            vae_path is not None or tcd_encoder_path is not None
        ), "Please select either VAE (full model) or TCD encoder (tiny / tiny_long)"

        # Tiny / Tiny long: 走 TCDecoder 路线
        if tcd_encoder_path is not None:
            if tiny_long:
                print("[FlashVSR] init tiny-long pipeline")
                model = init_pipeline_long(
                    prompt_path,
                    proj_pt_path,
                    dit_path,
                    tcd_encoder_path,
                ...
                print("[FlashVSR] init tiny pipeline")
                model = init_pipeline_tiny(
                    prompt_path,
                    proj_pt_path,
                    dit_path,
                    tcd_encoder_path,
        ...
        # Full: 使用 VAE 路线
            ...
            decode_vae_path = (
                folder_paths.get_full_path("vae", decode_vae)
                if decode_vae != "none"
                else "none"
            )
            print("[FlashVSR] init full pipeline")
            model = init_pipeline(
                prompt_path,
                proj_pt_path,
                dit_path,
            ...
            raise Exception("Please select the VAE or TCD encoder")
...
class FlashVSR_SM_KSampler(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input(
                    "steps", default=1, min=1, max=10000
                ),
                io.Float.Input(
                    "cfg",
                    default=1.0,
    ...
    def execute(
        ...
        # 根据 model 上是否存在 TCDecoder 来判断是 full / tiny / tiny_long
        if hasattr(model, "TCDecoder"):
            if getattr(model, "long_mode", False):
                print("[FlashVSR] infer tiny long mode")
                images = run_inference_tiny_long(
                    model,
                    image,
                    seed,
                    scale,
                ...
                print("[FlashVSR] infer tiny mode")
                images = run_inference_tiny(
                    model,
                    image,
                    seed,
                    scale,
            ...
            print("[FlashVSR] infer full mode")
...
class FlashVSR_SM_VideoPathLoop(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Combo.Input(
                    "video_file",
                    options=["none", "webm", "mp4", "mkv", "gif", "mov"],
                ),
                io.Custom("NUMBER").Input(
                    "reset_bool", optional=True
                ),
            ],
            outputs=[
                io.Video.Output(),
    ...
    def execute(
        ...
        **kwargs,
        ...
        video_path = (
            PureWindowsPath(video_dir).as_posix() if video_dir else None
        )
        ...
        return io.NodeOutput(
            VideoFromFile(selected_path), result, seed, filename
        )

    @classmethod
    def fingerprint_inputs(cls, **kwargs):
        # 让 ComfyUI 每次都重新执行（不缓存）
        return ""
...
async def comfy_entrypoint() -> FlashVSR_SM_Extension:
    # ComfyUI 调用此函数来加载扩展和节点
