...
from .utils.utils import Causal_LQ4x_Proj   # v1.1: Causal proj
import folder_paths
from comfy.utils import common_upscale

# =============== VRAM meter (minimal, low-intrusion) ===============
VRAM_LOG = os.environ.get("VRAM_LOG", "1") == "1"   # set 0 to disable
VRAM_CSV = os.environ.get("VRAM_CSV", "vram_log_v11_full.csv")
...
def _move_vae(pipe, device: str):
    ...
    # diffusers AutoencoderKLWan 或其它：常规 .to() 就行
    if cls_name != "WanVAE":
        with suppress(Exception):
            vae.to(device)
        return
...
def prepare_input_tensor(
    ...
    if isinstance(path, torch.Tensor):
        ...
        for img in pil_list:  # 修正 upstream 版本的 bug: 不再用 i 再索引
            img = img.convert("RGB")
    ...
    # 2) 帧图片文件夹
        ...
        frames = []
        for p in paths:
            # 修复 upstream 的 with Image.open(p).convert('RGB') 写法
...
# ================= v1.1 专用 pipeline init =========================
def init_pipeline_v11(
    prompt_path,
    ...
    """
    v1.1 版本：
    - 使用 Causal_LQ4x_Proj 作为 LQ 投影
    - 其它结构与 full pipeline 基本一致，只是 proj 更“因果”
    """
...
def run_inference(
    ...
    with torch.no_grad():
        ...
        def _decode_on(device_for_decode: str, use_segment_fallback: bool = True):
            # 只把 VAE 搬到目标 device；其它模块已 offload
    ...
    if save_vodeo_:
        save_video(
            ...
            os.path.join(
                ...
                f"FlashVSR_v11_Full_seed{seed}.mp4",
            ),
            fps=fps,
            quality=6,
        )
